...
/// `expected_resources`
fn test_one_end_process_stack__pop_and_terminate(
    file: &str,
    ...
    let project = Rc::new(Project::from_file(file, 1));
    println!("project {:?}", project);
    let res_vec = std::iter::repeat(0).take(project.nb_resource() + 1)
    		.collect::<Vec<usize>>();
	...
	for (process_name, nb_process) in processes {
		...
		end_process_stack = end_process_stack.add_processes__next_turn(
				project.clone(), process.borrow().get_index(), nb_process);
	}
