...
impl Parser {
    ...
    fn rule_optimisation(&mut self) -> bool {
        ...
        if carry_on {
            carry_on = self.tok_is_type(TokenType::SemiColon) && self.tok_is_type(TokenType::Word);
            if carry_on {
                opt.push(self.get_tok_content(-1));
            }
        }
        to_return = to_return && self.tok_is_type(TokenType::CloseParenthesis) &&
    ...
    fn rule_name_number_list(&mut self) -> bool {
        let old_state = self.save_state();
        let mut to_return = self.tok_is_type(TokenType::OpenParenthesis) && self.rule_name_number();
        let mut carry_on = to_return;
        while carry_on {
            carry_on = self.tok_is_type(TokenType::SemiColon) && self.rule_name_number();
        }
        to_return = to_return && self.tok_is_type(TokenType::CloseParenthesis);
        self.restore_state(to_return, old_state);
        to_return
    ...
    fn rule_process(&mut self) -> bool {
        ...
        let to_return = self.tok_is_type(TokenType::Colon) && self.rule_name_number_list();
        if !to_return {
            self.restore_state(to_return, old_state);
            return false;
        }
        ...
        let to_return = self.tok_is_type(TokenType::Colon) && self.tok_is_type(TokenType::Number) &&
                        self.tok_is_type(TokenType::EndLine);
        if !to_return {
            self.restore_state(to_return, old_state);
            return false;
    ...
    fn rule_empty_line(&mut self) -> bool {
        let old_state = self.save_state();
        let to_return = Parser::optional(self.tok_is_type(TokenType::Comment)) &&
                        self.tok_is_type(TokenType::EndLine);
        self.restore_state(to_return, old_state);
        to_return
    }
    ...
    pub fn parse(to_parse: &String)
        ...
        while carry_on && !parser.reached_end() {
            carry_on = parser.rule_empty_line() || parser.rule_initial_stock() ||
                       parser.rule_process() || parser.rule_optimisation();
            num_line += 1;
        }
